...
| 4.1 S2 prune + assert_tip_match + ASR | ⛔ 绝对第一优先级 | 未完成前停止 deep-root narrative、trait ASR 定稿、Phase 5 节点锁定 |
...
| 4.2 AA vs 3Di tree | ✅ 完成 | `tree_comparison.tsv/md`; 作为 QC3 正交证据保留 |
| 4.x cross-scenario ASR sensitivity | ⬜ 待做 | 脚本接口已补；待 S2 ASR 后运行 |
| 4.x orthogonal trait encoding | [/] 进行中 | `module_feature_registry.tsv`, `module_feature_matrix.tsv`, `panel35_feature_calibration.tsv` 已生成 |
...
1. `S2 prune -> assert_tip_match -> ASR`
2. `cross-scenario ASR sensitivity` on `S1/S2/S4a/S4b`
3. 35-panel feature calibration + annotation sensitivity
4. 在 repaired S4 与 S2 ASR 完成后，再决定是否触发 `S5 nonreversible/rootstrap`
